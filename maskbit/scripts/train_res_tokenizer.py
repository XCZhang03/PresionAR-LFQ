...
def main():
    ...
    logger = setup_logger(name="RQGAN", log_level="INFO", output_dir=config.experiment.logging_dir)
    ...
    if config.training.use_ema:
        ...
        def load_model_hook(models, input_dir):
            load_model = EMAModel.from_pretrained(os.path.join(input_dir, "ema_model"), model_cls=RQModel, config=config.model.vq_model)
    ...
    # EVALUATION STUFF.              #
    ##################################
    num_codebook_entries = model.codebook_size
    evaluator = ResidualTokenizerEvaluator(
    ...
    print("Using accelerator with rank", accelerator.process_index, "of", accelerator.num_processes)
    ...
    if config.experiment.init_checkpoint != '' and os.path.exists(config.experiment.init_checkpoint):
        ...
        if not resume_lr_scheduler:
            accelerator._schedulers = [lr_scheduler, discriminator_lr_scheduler]
        if dont_resume_optimizer:
            accelerator._optimizers = [optimizer, discriminator_optimizer]
...
def generate_images(model, original_images, fnames, accelerator, global_step, output_dir, logger_type):
    logger = get_logger(name="RQGAN", log_level="INFO")
