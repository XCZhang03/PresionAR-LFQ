...
class ResidualQuantizer(nn.Module):
    ...
    def __init__(self,
        ...
        self.quantizers = nn.ModuleList([codebook0 for _ in range(num_quantizers)])
        # else:
    ...
    def quantize(self, x, num_levels: int = None):
        r"""
        ...
        all_results = []
        if not num_levels: num_levels = self.num_quantizers
        for i in range(num_levels):
            quant, result_dict = self.quantizers[i](residual_feature)
            # residual_feature.sub_(quant.detach())
            residual_feature = residual_feature - quant.detach()
        ...
        return quant_list, all_result_dict

    def forward(self, x, num_levels: int = None):
        quant_list, result_dict = self.quantize(x, num_levels)
        quants_trunc = quant_list[-1]
        quants_trunc = x + (quants_trunc - x).detach()

        return quants_trunc, result_dict
    ...
    def get_codebook_entry(self, indices):
        ...
        quants = [self.quantizers[i].get_codebook_entry(indices[i].squeeze(0)) for i in range(N)]
        quants = torch.stack(quants, dim=0).sum(dim=0)
        return quants
...
if __name__ == "__main__":
    ...
    x = torch.randn(1, 10, 32, 32).requires_grad_()
    quantized, results_dict = quantizer(x)
    print(quantized.shape)
    codes = results_dict["min_encoding_indices"]
    print(codes.shape)
    embed = quantizer.get_codebook_entry(codes).permute(0, 3, 1, 2)
    print(embed.shape)
    print((torch.abs(embed-quantized).max()).item())
    assert torch.allclose(quantized, embed, atol=1e-6)